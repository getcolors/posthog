...
import { posthogWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify single-node PostHog\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(posthogWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(posthogWorkflow, defaultArgs(args));
  }
  const result = await run(...args);
  if (result["red/err"]) {
    ((result["red/exit"] ?? 0) === 0 ? console.log : console.error)(result["red/err"]);
  }
  return process.exit((result["red/exit"] as number | undefined) ?? 0);
}
